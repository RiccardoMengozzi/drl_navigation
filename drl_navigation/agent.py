...
def main() -> None:
    try:
        ...
        if LOAD_MODEL:
            ...
            with open(f"{log_path}wandb_id.txt", "r") as f:
                run_id = f.read().strip()
            ...
            with open(f"{log_path}wandb_id.txt", "w") as f:
                f.write(run.id)

        checkpoints_path = os.path.join(workspace_dir, "src", package_name, f'rl_models/checkpoints/{run.name}/')
        best_model_path = os.path.join(workspace_dir, "src", package_name, f'rl_models/best_model/{run.name}/')
